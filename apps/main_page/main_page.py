...
from components import nav, world_map
...
layout = dbc.Container(
    [
        nav.Nav(active="Home").render(),
        dcc.Markdown("# Countries by: population", style={"textAlign": "center"}),
        world_map.Map().render(),
        dcc.Markdown("# Countries table", style={"textAlign": "center"}),
        dbc.Label("Show number of rows"),
        row_drop := dcc.Dropdown(
            value=10, clearable=False, style={"width": "35%"}, options=[10, 25, 50, 100]
        ),
                ...
                dbc.Col(
                    [
                        country_drop := dcc.Dropdown(
                            [x for x in sorted(df.country.unique())], multi=True
                        )
                    ],
                    width=3,
                ),
